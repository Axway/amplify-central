...
6. In Configure Specifications, if one or more linked API resources support partial exposure (for example, unstructured, OAS2, or OAS3 services), you can selectively include or override portions of the API specification. Changes made in this step only affect how the API is exposed within the asset and do not modify the source API service.
By default, the Exposure column is set to **Full** for all services. Click Edit Specification menu option on the service, the edit specification side blade opens. After loading a partial specification file, the exposure column changes from **Full** to **Partial**

If partial exposure is enabled for any linked API, Auto-Release will be disabled in the Release Management step, as manual control is required for publishing updates.

7. Add the following Release Management information and then click **Next**:
