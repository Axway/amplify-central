...
{{< alert title="Note" color="primary" >}}**Important**: Ensure the **Enabled** toggle is set to **Show**. Setting it to **Hide** disables the custom homepage and prevents it from being displayed.
Changes are reflected dynamically in the preview, which can be toggled between.{{< /alert >}}
...
{{< alert title="Note" color="primary" >}}Actions are executed by clicking the respective buttons. Changes appear immediately in the preview.{{< /alert >}}

### Add an element

1. Click the **+** to add a new element.
...
Click **Edit** to open the element configuration screen specific to the element type.

### Delete an element

Click **Delete** to delete the selected element and reorder the remaining elements accordingly.

### Reorder an element

Click **Reorder** to drag and drop the element to update the order of the elements.
...
{{< alert title="Note" color="primary" >}}Changes are *not applied* until you click **Save**. All edits, imports, or resets take effect only after saving.{{< /alert >}}
...
Each of the following available elements can be configured independently, and multiple instances of any element type can be added.
...
#### Page Header configurable properties
...
* Multiple card elements can be used
 ...
 A Card (Content) element can include any combination of the following elements:
...
#### Categories configurable properties
...
#### Products configurable properties
...
#### Button configurable properties
