...
* **OAuth Client support for FDX standards** - Registering and un-registering of an OAuth Client to meet FDX standards is now supported.  A new token is required for each register and un-register call.  
...
* **Consumer can subscribe to an archive plan** - Due to the introduction of the archive status, the consumer could select an archive plan when subscribing directly from the plan page. Now, a plan must be active for a consumer to subscribe to it.
