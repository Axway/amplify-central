...
* **Category hierarchy** - When configuring the categories, it is now possible to set a parent to a category. You make a hierarchy up to 3 levels. Once a category is assigned to a parent, it is no more possible to change its visibility nor its ownership: it will follow the definition of its parent. The Category listing shows the Parent of each category (if exists) as well as the number of its children.
