...
* **Credential Scope Display** - When only one scope is available for a Marketplace credential, the scope would not be visible.  The display of scope selection was fixed.
* **Null pointer exception** - Fixed a null pointer exception as a result of a unsupported Filebeat subcommand.
...
* **Category hierarchy** - When configuring the categories, it is now possible to set a parent to a category. You can create a hierarchy up to 3 levels. Once a category is assigned to a parent, it is no longer possible to change its visibility or its ownership; it will follow the definition of its parent. The Category listing shows the Parent of each category (if exists) as well as the number of its children.
...
* **Filter per resource State** - The product details screen has been enhanced to display the state of an API resource (design, experimental, pre-released, stable, deprecated, archived). In addition, providers can filter the product catalog by the API resource state..
* **Category hierarchy filter** - When visualizing a category, it is possible that this category is a parent for others category. The product associated with the category can be filter using the children categories. More information [here](/docs/manage_marketplace/consumer_experience/searching#categories-search)
