...
* **Free plan quota not created with a default value** - Now, the default limit value 1 is taken into account and correctly creates the plan quota.
...
* **Category works in multi Marketplace context** - With the introduction of multiple Marketplaces, it is now possible to use a category in one or more Marketplace. The category can be featured in a specific Marketplace.
