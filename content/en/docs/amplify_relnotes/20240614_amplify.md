...
|             | APIGOV-26232 | **Issue**: UI is able to unlink an asset from a product even when the asset resource is used in the product plan quota. <br />**Resolution**: the unlink button in now disable in case the asset is already in used in a product plan quota. A tooltip also indicate why the button is disabled. |
...
| Case ID   | Internal ID  | Description                                       |
|-----------|--------------|---------------------------------------------------|
| 01598875  | APIGOV-28013 | **Issue**: Makretplace PKCE tryout with external OAuth not working. <br />**Resolution**: The authorization flow for oauth has been enhanced to send the PKCE header in the token request. |
