...
* **Usage Audit Log** - A new Usage Audit log that is enabled by default to keep track of successfully communicated platform usage counts.  The usage audit log will be stored on the local file system for at least 365 days or until the congiufration file size limit has been reached.  
...
|---------    |--------------|---------------------------------------------------|
| 01567300 <br />01578477   | APIGOV-27915 <br />APIGOV-27886 | **Issue**: The Discovery and Traceability Agent were restarting with a concurrent map read and map error <br />**Resolution**: A fix was made to concurrent map reads and writes  |
...
| Case ID     | Internal ID  | Description                                       |
|---------    |--------------|---------------------------------------------------|
|             | APIGOV-27892 | **Issue**: Azure API Service names were not being sanitized when stored in the Service Registry <br />**Resolution**: A fix was made so the API Service revision names which had capital letters were converted to lower case letters for storage in the Service Registry  |
...
| 01544107 <br />01537104    | APIGOV-26808 | **Issue**: Cannot archive an Asset Release <br />**Resolution**: The was fixed as part of the Unable to add an asset to a product issue  |
