...
* **Unpublished API** - The Discovery Agent will only discovery published front end APIs.   Unpublished front end APIs are ignored by the Discovery Agent.
...
* **Base64 Encoding** - The Istio Agents now support the use of base64 encoded secrets to create k8s secrets for the agents.
...
* **Support contact office hours** - For support contact, you can define the office hour intervals when consumers can contact you.
* **Multi marketplace support** - it is now possible to publish a product into one or more configured marketplaces, and configure the visibility of the product per each marketplace.
...
* **Application page enhancement** - The application details page has been enhanced to address some usability issues.
* **Corrupted product are not visible in marketplace** - A product containing an API that is corrupted (i.e api has been deleted) is no longer shown to consumers that do not have a subscription to the product.
...
* **Non logged users have access to credentials** - Non logged users can access the credentials screen - Now, only a logged in users can see the credentials screen.
