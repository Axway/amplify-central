...
* **Provider engagement** - Provider has an overview of the total services, assets and products he has deployed. Each figure is declined by the object status (Draft / Active / Deprecated / Published / Archieved). The provider engagement is cross team but can be filter to a specific team. It can also be filter for a specific time frame. Refer to Business Insights > Provider engagement
