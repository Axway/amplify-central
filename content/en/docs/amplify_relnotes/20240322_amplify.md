...
* **Average Response Time calculation** -  A fix was made to all Embedded Traceability Agents to accurately calculate the average response time metric.
...
* **Draft Asset versions** - A UX improvement was made to only create a draft when a grouped API resource has changed.  Other edits will take effect immediately without requiring a draft to be created and activated to take effect.
