...
* **Invoke Policy Mapping to Amplify Marketplace Credential** - An enahancement to enable the mapping of Invoke Policy to an Amplify Marketplace credential has been added. Support for API Key, Basic Auth, and Oauth have been added.  For more details, see [Agent Variables](<https://docs.axway.com/bundle/axway-open-docs/page/docs/connect_manage_environ/connect_api_manager/agent-variables/>).
...
* **Category update** - it is no longer possible from the UI to edit a category and select it as a parent.
...
* **Large WSDL file cannot be displayed without crashing the browser** - * **Large WSDL file cannot be displayed without crashing the browser** - the component that displays the WSDL content has been optimized for large file rendering.
* **Marketplace is not rendering images** - an issue has been fixed that prevented images to be displayed in protected Marketplace or for products with restricted visibility.
...
* **Product rating filter still available even when product rating is not activated** - the product rating filter is now visible only when the ratings and reviews are enabled on the Marketplace.
