...
* **Featured categories are overlapping** - The display of five featured categories has been enhanced. Now, the category names are displayed correctly.
* **Long support contact names are displayed properly in the Marketplace** - Long support contact names are truncated to fit on the screen, but a tooltip is now available to shows its entire value.
...
* **Application details screen does not allow credentials to be viewed** - The view credentials button was closing the credentials panel. Now, the view credentials button displays the credentials panel.
* **The navigation section in documentation is broken when using special characters ('?', '#', '/'...)** - Now, special characters are accepted in the documentation section without breaking the navigation.
