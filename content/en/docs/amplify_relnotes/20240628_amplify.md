...
|---------    |--------------|---------------------------------------------------|
|             | APIGOV-28128 | **Issue**: An issue was found with the parsing of OAS2 specifications. <br />**Resolution**: A fix was made to the parsing logic.  For example, an OAS3.1 specification file was discovered as Unstuructured is now corectly represented in the agents. |
|             | APIGOV-28081 | **Issue**: An issue was found with offline mode for the Axway API Management Gateway traceability agent sending error messages with 'reportALLErrors' variable set to true. <br />**Resolution**: A fix was made to no longer send error message while in offline mode and ignore the 'reportAllErrors' variable set to true in this case. |
| 01603732    | APIGOV-28137 | **Issue**: An issue was found with the setting of credential type for SOAP APIs which were previously discovered. <br />**Resolution**: A fix was made to properly set the credential type for SOAP APIs. |
...
| 01604349    | APIGOV-28152 | **Issue**: A security vulnerability was found with an Alpine docker image used in the Discovery and Traceability release. <br />**Resolution**: The Alpine docker image library version was updated to a newer released version (3.20.1) which Alpine has fixed this security vulnerability. |
...
| Case ID     | Internal ID  | Description                                       |
|---------    |--------------|---------------------------------------------------|
| 01603233    | APIGOV-28126 | **Issue**: The SaaS (embedded) agents were not setting the API Service owner per the team ownership setting. <br />**Resolution**: The SaaS (embedded) agents will set the team ownership of adiscovered API Service per the team ownership setting. |
...
There are not bug enhancements in this release.
...
There are feature updates in this release.

#### Bug fixes for Service Registry

| Case ID     | Internal ID  | Description                                       |
|-------------|--------------|---------------------------------------------------|
| 01400212    | APIGOV-27808 | **Issue**: A developer (non-central admin) role could not add an endpoint and set the owner on the Service details page. <br />**Resolution**: A fex was made to allow the setting of the owner by a developer (non-central admin) role when adding an endpoint. |
|             | APIGOV-28228 | **Issue**: A mock endpoint could not be created for Runtime Managed APIs. <br />**Resolution**: A fix was made to allow the creation of mock endpoints for OAS2/OAS3 Runtime Managed APIs discovered by Agents. |
...
| Case ID     | Internal ID  | Description                                       |
|-------------|--------------|---------------------------------------------------|
|             | APIGOV-28114 | **Issue**: The WebUI for the Stages page did not allow an existing stage Marketplace Visibility for a consumer organization to be set to 'None'. <br />**Resolution**: A fix was made to allow the editing of the stage Marketplace Visibility to be set to 'None'. |
|             | APIGOV-27776 | **Issue**: The WebUI for the Environment details page did not display more information about the agent status when "Unhealthy". <br />**Resolution**: A fix was made to display the more detailed agent status information from the message details when hovering the cursor over the "Unhealthy" status. |
|             | APIGOV-28183 | **Issue**: The WebUI for the Compliance Profiles tab was displaying '0 {name}' and the text for the 'Upload Ruleset' button was off center. <br />**Resolution**: A fix was made to display the Compliance Profiles tab for these two display texts properly. |
|             | APIGOV-24067 | **Issue**: The WebUI when manually uploading an API Service allowed the user to select an API specification type which did not match the uploaded specification file. <br />**Resolution**: A fix was made to not allow the user to change the auto-detected API specification file after it has been uploaded. |
...
| Case ID     | Internal ID  | Description                                       |
|-------------|--------------|---------------------------------------------------|
| 01605023    | APIGOV-28182 | **Issue**: The WebUI for the Asset wizard, would not allow a Catalog Manager to add an API resource owned by Team B to be added to an asset in an environment owned by Team A.  <br />**Resolution**: A fix was made to allow an API resource visible from the selection list to be added to an asset in an environment owned by another team. |
|             | APIGOV-28142 | **Issue**: The WebUI for the Asset details page was not pre-selecting the latest asset release by the creation date. <br />**Resolution**: A fix was made to auto-selection to display the latest asset release based on the creation date. |
