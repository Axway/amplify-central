...
|--------------------------------------------|--------------------------|
| Axway API Management 7.6.2 SPx and 7.7 SPx | DA=v1.1.77 / TA=v1.1.78  |
...
**Audit Logs** - The Agent SDK has been enhanced with the addtion of two additional audit logs.  
- A Metric audit log has been added to all traceability agents and enabled by default to track the metric events based off traffic through the gateway.
...
* **Transaction Audit Log** - The traceability agent has added a Transaction Audit log to track the correlation IDs of each transaction the agent has seen and recorded.  This audit log can be used to reconcile trasactions seen versus communicated to the Amplify platform.
For more details, refer to [Understand the Traceability Agent audit logs](https://docs.axway.com/bundle/amplify-central/page/docs/connect_manage_environ/connected_agent_common_reference/traceability_audit_logs/).
...
* **Metric event limit** - The number of metric events communicated has been limited to 1 per api/status code for every 15 minutes.  This fixed a situation where a large number metric events could be sent too often. 
...
* **SwaggerHub installation** - The 'install agents' command has been enhanced to support the installation of the embedded SwaggerHub disovery agent.
...
* **Service Registry Display** - The Service Registry display has been improved with tabs for All, Design, Runtime Managed and Runtime Unmanaged APIs for quick filtering.  The Runtime Unmanaged APIs will not display any results until Runtime Compliance is enabled for you organization,
...
##### Enhancements for Asset Catalog

There are no enhancements in this release.

##### Bug fixes for Asset Catalog

* **Maxiumum Asset entitlement** - A bug fix to the UI was made to disable the 'Add New' button when using the sideblade to add a new asset when the maximum entitlement was reached. 
...
* **UI - Product release type unknown if there are over 20 releases** - In case you have more than 20 product release, some product release information were missing because of the paginated query. Now, all information is available regardless of the number of releases.
