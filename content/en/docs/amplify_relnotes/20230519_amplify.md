...
* **Credential management** - A new section "Credential management" has been added to the environment page. This section allow to define the credenitals expiration date the discovery agent will use when provisoning  credentials, if the agent must deprovisioned automatically a credenital that expires and how often the consumer will receive notification prior to the credentials expiration. This new options allows to remove the `CENTRAL_CREDENTIALS_DEPROVISIONONEXPIRE` and `CENTRAL_CREDENTIALS_EXPIRATIONDAYS` variables in the discovery agent latest release.
...
* **Subscribing from plan details does not pre-fill the plan drop-down** - When subscribing from the product plan details view, the plan is now automatically selected on the subscription screen.
