...
* **Credential management** - A new section "Credential management" has been added to the environment page. This section allow to define the credenitals expiration date the discovery agent will use when provisoning  credentials, to define if the agent must deprovisioned automatically a credenital that expires and how often the onsumer will receive notification prior to the credentials expiration. This new options allows to remove the `CENTRAL_CREDENTIALS_DEPROVISIONONEXPIRE` and `CENTRAL_CREDENTIALS_EXPIRATIONDAYS` variables in the discovery agent latest release.
