...
* **Subscription Migration** - A fix was made to handle the migration of existing Marketplace subscriptions to use the correct usage plan.
...
* **Quota Provisioning feature** - The Product plan quota can be provisioned on an AWS Gateway. This capability allows the Product to have a plan limiting the quantity of usage of product per a specified time interval. Refer to Azure / Product / Policy.
...
* **Configurable IDP settings** - The Apigee Edge agent can have configurable IDP setttings to support a custom login URL.

##### Bug fixes for Apigee Edge Gateway

* **CPU/Memory Utilization** - The Apigee Edge agent can now be configured to discover a maximum number of API Services in a batch to reduce the CPU/Memory usage.
...
* **Incorrect warning displayed** - A warning was wrongly shown in Marketplace when creating a subscription when the owner is not selected yet.
* **Error message when subscribing is not clear** - The error essage displayed when the user is not allowed to create subscription with paid plan.
