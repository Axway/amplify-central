...
* **Resource version not visible when creating a product plan** - The description of the resource available in the plan quota has been enriched with the following pattern: {AssetTile}/{resourceTitle} - {stageTitle} - {stateTitle} - {resourceVersion}. StageTitle and stateTitle mmay be empty.
* **Line break issue in description fields** - the carriage return is now correctly display in Asset and Product description field.
...
* **HSTS Max age too low** - HSTS max agen has been increased to 12 months.
* **Not all swagger endpoints are visible in the methods section** - On product method screen, the Server dropdown shows now all the endpoint available.
...
* **Number of products displayed doesn't match the displayed products** - When a product is in error (ie when all its resources are corrupted), the product count from the Browse product page was wrong. Now product in error are no more displayed in the Browse product page and the product count is accurate.
