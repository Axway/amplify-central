...
* **Environment deletion** - The environment deletion action for Embedded agent environments now works as designed. Environments are no longer stuck in the deletion pending state.
...
* **API Service endpoint discovery** - API Endpoints in proxy mode with the correct scheme are now discovered.
* **Product access revoked** - Product access is now revoked if the access request is deprovisioned.
...
* **Document Resource resources are not shown correctly in the UI when switching versions** - When the user switches from one product version to another, the documentation and resources are now displayed correctly.
