...
In order to better filter transactions related to the services in the mesh, certain resources need to be created for each service running on the mesh - namely APIService, APIServiceRevision and APIServiceInstance. The APIService and APIServiceInstance needs to include the sub-resource "x-agent-details" with "externalAPIID" property in its definition.
