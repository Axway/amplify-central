...
This CloudWatch Log Group will be used by both agents. Upon discovery of a Rest API stage, the Discovery Agent configures the logging to send details to CloudWatch. The Traceability Agent then watches the CloudWatch for new entries and sends usage, metric, and transactional data to Enterprise Marketplace.
