...
| AGENTFEATURES_IDP_TITLE                     | The title of the Credential Request Definition that will be shown to consumers in Amplify Marketplace.                                                                                                                                                                                                                                    |
...
| CENTRAL_METRICREPORTING_SCHEDULE               | The schedule at which the agent reports API metrics to Amplify and cache usage numbers (default value: `@hourly`). Takes precedence over replaced variable CENTRAL_USAGEREPORTING_INTERVAL.                                                                                                                                     |
...
| TRACEABILITY_SAMPLING_PERCENTAGE               | The percentage of all transactions that are sent to Amplify. A value of `0` reports no traffic (default value `0`), maximum value `10`. If the value is set higher than the maximum value, then the agent will use the default value.                                                                                           |
...
| TRACEABILITY_SAMPLING_ONLYERRORS               | When true, only API transactions that have an http error code will be sent to Amplify (default value: `false`).                                                                                                                                                                                                                 |
