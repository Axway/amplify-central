...
| AGENTFEATURES_IDP_AUTH_ACCESSTOKEN          | The token (initial access token, Admin API Token, etc.) to be used by the Agent SDK to authenticate with the OAuth identity provider. For Okta, this must be an [Admin API token](https://developer.okta.com/docs/guides/create-an-api-token/main/) with permissions for application, group, and authorization server policy management. See [Marketplace provisioning](/docs/connect_manage_environ/marketplace_provisioning) for details. |
