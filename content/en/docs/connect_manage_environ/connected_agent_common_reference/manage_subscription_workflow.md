...
{{< alert title="Note" color="primary" >}}Depending on the poll interval settings for the Discovery Agent, it will take a little time from when the user subscribes an API to an application/usage plan/subscription until the subscription state shows **Active** in Marketplace. This is because of the time it takes to send events back and forth between the Gateway and Amplify.{{< /alert >}}
...
1. A consumer initiates unsubscribe:
   ...
   2. Unsubscribe from the active subscription.
