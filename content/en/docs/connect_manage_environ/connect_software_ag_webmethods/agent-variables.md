...
| WEBMETHODS_GATEWAYHOST            | Overrides the hostname of discovered API endpoints published to Amplify Central. Use when the gateway is behind a load balancer or custom domain. Only the hostname and port are used — the scheme is determined by webMethods. Must be a valid URL if set.                                                                                     |
...
| WEBMETHODS_USESEARCHENDPOINT      | When set to `true`, the Discovery Agent uses `POST /apigateway/search` instead of `GET /apigateway/apis` to retrieve the initial API list. Default: `false`.                                                                                                                                                                                    |
| WEBMETHODS_SEARCHBATCHSIZE        | Optional batch size for /search queries. For Discovery, use only when `WEBMETHODS_FILTER` is set and there is a big gap between total Gateway APIs and expected discovered APIs; otherwise it may add unnecessary Gateway load. For Traceability, it's safe to use and encouraged. Suggested range: `10` to `50`. Default: `0` (disabled).      |
...
| WEBMETHODS_EXECUTIONTIMEOUT       | The maximum duration for an execution processing (default: `1h`, minimum: `5m`).                                                                                                                                                                                                                                                                |
