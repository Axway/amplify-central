...
| INPUTS_IGNORE_OLDER                           | When set the agent will not track files that were modified before the set duration ago (default value: `1h`). See [ignore_older](https://www.elastic.co/guide/en/beats/filebeat/current/filebeat-input-log.html#filebeat-input-log-ignore-older).                                                                                          |
...
| DEDUPLICATION_EXPIRY    | Time to live for each correlation ID in the agents memory (default value: `5m`)                                                      |