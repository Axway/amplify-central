...
When a Front End Proxy is secured by an authentication scheme(OAuth, Basic Auth, API Key, OAuth External), the agent manages the credential provisioning based on respective credential type. This behavior can be overridden to allow custom processing for the credential by configuring a mapping for the credential type to externally managed credential request definition.
