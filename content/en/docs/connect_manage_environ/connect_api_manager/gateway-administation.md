...
`EVENT_LOG_INPUT`: Used to turn on or off the event log input for the Traceability Agent. Defaulted is `true`, set to false when using [open traffic](/docs/connect_manage_environ/connect_api_manager/#sourcing-gateway-traffic-from-open-traffic-file-s).
