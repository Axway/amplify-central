...
* You will need a user account for Amplify
* Import your API as an API proxy in Amplify (see [Register an API](/docs/saas_api_gateway/quickstart/#register-an-api))
...
This script creates an RSA key pair. The private component must be kept secret and is used for signing authentication tokens. The public component is added to the application in Amplify. The public key is converted to a JWK and Amplify uses it to validate the authentication tokens.
...
Copy and paste the **Application ID** and **Key ID** from the API proxy in Amplify.
...
* Use the default public key from jwt.io as the **JWT Key** for your app in Amplify or create a public key as detailed in [Generate a key pair](#generate-a-key-pair).
* To create the JWT token, you need the **Application ID** and **JWT Key ID** from the API proxy in Amplify.
* Use the generated JWT token from jwt.io as the **JWT Token** to test your API in Amplify.

{{< alert title="Caution" color="danger" >}} This method is for testing only as it requires you to share your private key with a third party website.{{< /alert >}}
...
4. Edit the Header section. Copy and paste the **JWT Key ID** from the **Test Methods** tab of the API proxy in Amplify as the `kid` field. For example:
...
5. Edit the Payload section.
    * Copy and paste the **Application ID** from the **Test Methods** tab of the API proxy in Amplify as the `sub`, `aud`, and `iss` fields.
...
You have learned how to secure your API with a JWT token in Amplify.
